...
The following example shows how to modify the spacing of x-axis breaks. The default displays all breaks, i.e. each x-axis value included in `data`. The `break_interval` argument sets the interval between breaks, given as a multiple of the interval between data points in `data`. 
...
my_chart <- rchart::chart(data)
my_chart_breaks <- rchart::chart(data, break_interval = 5)

my_chart$chart_param + ggplot2::ggtitle("Default: All Breaks")
my_chart_breaks$chart_param + ggplot2::ggtitle("break_interval = 5")
```
