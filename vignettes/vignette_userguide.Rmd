...
my_chart <- rchart::chart(data, 
                          append = "_appendName", # Will apend names to each chart
                          folder = "myfolder", # Will create a new folder if needed
                          save = T, # This is True by default but can be turned off for rmakdown docs
...
my_chart <- rchart::chart(data) # For just this chart rchart::chart(data, chart_type="param_absolute")
...
my_chart <- rchart::chart(data, scenRef="scen1") # So that all plots are not shown
...
```{r, results = 'hide', eval=TRUE, echo=TRUE, warning=FALSE, error = FALSE, message = FALSE, fig.width=21, fig.height = 10}
library(rchart);
...
data = data.frame(region = c(rep("Spain",54),
                  ...
                  year = rep(rep(c("2010","2010","2020","2020","2030","2030",
                           "2010","2010","2020","2020","2030","2030",
                           "2010","2010","2020","2020","2030","2030"),3),2),
...
my_chart <- rchart::chart(data, scenRef = "scen1")
...
my_chart <- rchart::chart(data) # For just this chart rchart::chart(data, chart_type="region_absolute")
...
The following examples show two ways to modify the spacing of x-axis breaks. The default displays all breaks, i.e. each x-axis value included in `data`. The `breaks_x` argument is used to specify the number of desired breaks, while the `break_interval` argument sets the interval between breaks, given as a multiple of the interval between data points in `data`. If both arguments are given, `breaks_x` overrides `break_interval`. 

```{r, results = 'hide', eval=TRUE, echo=TRUE, warning=FALSE, error = FALSE, message = FALSE, fig.width=7, fig.height = 5}
library(rchart)
data = data.frame(scenario = c(rep("scen1", times = 21),
                               rep("scen2", times = 21),
                               rep("scen3", times = 21)),
                  year = rep(seq(from = 2010, to = 2030, by = 1),3),
                  param = c(rep("param_1",63)),
                  value = c(seq(31,45,length.out = 15), seq(46,60, length.out = 6),
                            seq(38,56,length.out = 10), seq(58,50, length.out = 11),
                            seq(40,49,length.out = 10), seq(50,28, length.out = 11)))

my_chart <- rchart::chart(data)
my_chart_breaks_x <- rchart::chart(data, breaks_x = 10)
my_chart_break_interval <- rchart::chart(data, break_interval = 5)

my_chart$chart_param + ggplot2::ggtitle("Default: All Breaks")
my_chart_breaks_x$chart_param + ggplot2::ggtitle("breaks_x = 10")
my_chart_break_interval$chart_param + ggplot2::ggtitle("break_interval = 5")
```
